...
Memoria *inserir45(Memoria **raiz, Memoria *pai, Info **sobe, int inicio, int fim, int status, int *flag) {
    Memoria *maiorNO;
    maiorNO = NULL;
        ...
        if (ehFolha(*raiz)) {
            *flag = 1; // informa que foi inserido
            ...
            if (inicio < (*raiz)->info1->inicio)
                maiorNO = inserir45(&(*raiz)->esquerda, *raiz, sobe, inicio, fim, status, flag);
            else if ((*raiz)->numKeys == 1 || ((*raiz)->numKeys <= 2 && inicio < (*raiz)->info2->inicio))
                maiorNO = inserir45(&((*raiz)->esquerda_centro), *raiz, sobe, inicio, fim, status, flag);
            else if (((*raiz)->numKeys == 2) || ((*raiz)->numKeys <= 2 && inicio < (*raiz)->info3->inicio))
                maiorNO = inserir45(&((*raiz)->centro), *raiz, sobe, inicio, fim, status, flag);
            else
                maiorNO = inserir45(&((*raiz)->direita), *raiz, sobe, inicio, fim, status, flag);

...
int main() {
	Memoria *arvore = NULL,*maiorNo = NULL,*sobe = NULL; // Inicialize arvore como NULL
	int inicio, fim, status = 2;
	int flag = 0;
	char opc;
	...
	inserir45(&arvore,NULL,&sobe,inicio,fim,status,&flag,NULL);
	inicio = fim+1;
	while (fim != BLOCO) {
		...
		if (fim >= inicio && fim <= BLOCO) {
			status = rand() % 2;
			flag = 0;
			inserir45(&arvore, NULL, NULL, inicio, fim, status, &flag,&maiorNo);
			inicio = fim+1;
		} else {
